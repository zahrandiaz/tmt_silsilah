                        ...
                        <div class="space-y-6">
                            ...
                            <div>
                                ...
                                <select name="type" id="type" class="block mt-1 w-full border-gray-300 dark:border-gray-700 dark:bg-gray-900 dark:text-gray-300 focus:border-indigo-500 dark:focus:border-indigo-600 focus:ring-indigo-500 dark:focus:ring-indigo-600 rounded-md shadow-sm">
                                    <option value="info" @selected(old('type', $announcement->type) == 'info')>Biru (Info)</option>
                                    <option value="success" @selected(old('type', $announcement->type) == 'success')>Hijau (Sukses)</option>
                                    <option value="warning" @selected(old('type', $announcement->type) == 'warning')>Kuning (Peringatan)</option>
                                </select>
                            </div>

                            <div>
                                <label for="is_active" class="inline-flex items-center">
                                    <input id="is_active" type="checkbox" class="rounded border-gray-300 text-indigo-600 shadow-sm focus:ring-indigo-500" name="is_active" value="1" @checked(old('is_active', $announcement->is_active))>
